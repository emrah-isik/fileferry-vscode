...
**SSH Credentials** (`globalStorageUri/credentials.json`) store connection details: host, port, username, auth method, optional private key path or agent socket path, an optional `useSshConfig` flag (the host is an `~/.ssh/config` alias, resolved at connect time), and an optional ordered `jumpHosts` list of other credential ids (first hop → last hop before the target; flat: validation rejects nesting from both sides, self-reference, and dangling ids). Secret fields (password, passphrase) are never in this file; they live in the OS keychain.
...
**Import from vscode-sftp (feature 35b):** `src/importers/vscodeSftp/` holds pure modules behind thin command glue, following the `ConfigMigration` shape (pure converter + injected dependencies). `parser` flattens the three `sftp.json` shapes (single object, array, `profiles`) into one `SftpJsonEntry` per future server, tolerates JSONC, and emits typed notices for what it drops. `mapper` turns entries into `{ config, credentials, report }` per the feature-35 field-mapping table; its only impure edges are two injected prompts (a missing password, a relative `remotePath`) where Esc skips the item. `ignoreTranslator` maps the gitignore dialect onto `excludedPaths` for the compatible subset only (plain names, simple globs, inner-slash paths prefixed with the `context`); negations, anchored, and trailing-slash rules are reported by name, never guessed. `report` renders the output-channel report and the one-line summary; `detection` decides the one-time offer (a `workspaceState` flag remembers *Not now*) and the `fileferry.vscodeSftpDetected` context key behind the Servers welcome-view hint. `commands/importFromVscodeSftp.ts` reads the file, inlines any `ignoreFile`, maps, saves credentials (keychain) before the config so no id dangles, refreshes, and reports. The import is additive: existing servers are never touched, duplicates (host + port + username + root path) are skipped, the default server is set only when none is set, and `sftp.json` is never modified. Imported `hop` chains become stored credentials with `jumpHosts` (never the `sshconfig:<alias>` synthetic path).
...
| Webview → Extension | Upload History | `ready`, `filter` (`{ serverId?, result?, search?, trigger? }`; `search` matches local and remote paths), `clear` |
| Extension → Webview | Upload History | `init` (`{ entries, servers }`), `filtered` (`{ entries }`), `cleared`. Each entry carries a computed `displayPath` (`historyDisplayPath`: the remote path for `remote-edit`, `remote-create` and `remote-duplicate`, whose `localPath` is a temporary copy; otherwise the local path) |
...
- Host key verification applies to every SSH connection: SFTP sessions (`SftpService.connect()` takes the `HostKeyProvider` from the registry below unless the caller passes an explicit `hostVerifier`), jump-host dials, and the SSH terminal's raw client (the latter two through `createCredentialDialer`). FTP/FTPS have no SSH host key and pass nothing.
...
**`~/.ssh/config` and route resolution** (`src/ssh/SshConfigResolver.ts`, `src/ssh/routeResolution.ts`, `src/ssh/routeDescription.ts`, all vscode-free; feature 19 + 18b):

- `resolveHostAlias(alias, deps)` parses the user's SSH config with OpenSSH's *first obtained value wins* rule over `Host` blocks (`*`/`?` patterns) and returns `HostName`/`Port`/`User`/`IdentityFile` (tilde-expanded) plus, since 18b, the raw `ProxyJump` (absent for `none`) and `ProxyCommand` values, only one of the two, whichever the block specifies first, as OpenSSH does. A `Match` line ends the preceding `Host` block and is otherwise not evaluated (M6); `Include` is unsupported. `applySshConfig(credential, resolved)` is the pure merge (config wins, credential value is the fallback, the alias stays the host when there is no `HostName`). `ResolverDeps` (config path, home dir, file reader, local user) is how tests inject a config; production passes nothing.
- `resolveChain(alias, deps)` turns the alias's `ProxyJump` into `ResolvedHop[]` in connect order (Q4): each token (`[user@]host[:port]`, comma lists, bracketed IPv6) is looked up as its own `Host` block (its own `ProxyJump` is followed first, recursively), a literal user/port overrides the block's, a hop with no user anywhere gets the local login name (OpenSSH behaviour), the chain is capped at `MAX_PROXY_JUMP_HOPS` (8), and an alias cycle throws `SshConfigChainError`. A hop whose block carries a `ProxyCommand` is returned with it attached so callers can warn.
- `resolveRoute(credential, deps)` is the single decision every consumer makes: no `useSshConfig` → the credential's `jumpHosts` (credential ids); otherwise `applySshConfig` on the target and, when the credential has explicit `jumpHosts`, those (a `ProxyJump` on the alias is *ignored* and produces a note, Q5-1), else the `ProxyJump` chain (`ResolvedHop`s). `ProxyCommand` hosts (target or hop) are collected; `reportRouteNotices(route, providers)` logs each note and warns about each `ProxyCommand` host **once per extension session** through the registry `warn` sink, never executed (Q5-2). `SftpService.connect()` and `SshTerminal.connect()` both go through this pair (and accept `sshConfig: ResolverDeps` for tests), so a server is reached the same way by a deploy and by a terminal. `describeRoute(credential, credentials, deps)` renders the same decision as `local → hop… → target` for the Servers tooltip and the terminal banner (explicit hops by credential, a deleted one as `(missing jump host)`, config hops by resolved `user@host:port`, or the chain error in parentheses); the credential-panel summary (`describeResolution`, now given the credential's `jumpHosts`) shows the route, the explicit-wins note, and `ProxyCommand` notes.

**Jump-host chains** (`src/ssh/chainConnect.ts` + `src/ssh/JumpHostPool.ts`, both vscode-free, 18a-2a):

- When the resolved route has hops, `SftpService.connect()` calls `chainConnect(target, hops)` with `ChainHop[]`, each entry a credential id or a `ResolvedHop` from `ProxyJump` (18b). A credential id is looked up through the registry's `jumpHosts.resolveCredential` and, when that hop credential itself opts into `~/.ssh/config`, resolved independently with `applySshConfig` (R8-5; a `ProxyJump` on the *hop's* alias is logged as ignored: chains are flat, Q3). A config-derived hop is never stored: it becomes a synthetic `SshCredentialWithSecret` for this dial only, pooled under `sourceId` `sshconfig:<alias>` (so `evictBySourceId` never matches it, while the canonical `user@host:port` pool key still dedupes it against a stored credential for the same host), with its auth chosen per Q15: `IdentityFile` → key auth; else `resolveAgentSocket()` → agent; else keyboard-interactive through the registry provider (ssh2's default auth handler tries these in that order on one connection, and `tryKeyboard` is on for interactive dials); a background (`interactive: false`) dial then fails before dialing with `VerificationRequiredError` like any prompt-only hop, and with no prompt provider at all the chain fails with `hop N (<alias>): no IdentityFile, no agent, add IdentityFile or define it as a FileFerry credential`. An *All configured authentication methods failed* on a config hop carries a hint naming the three things such a hop can authenticate with. Every hop is leased from the pool, each hop *N*+1 dials over a fresh `forwardOut('127.0.0.1', 0, next.host, next.port)` from hop *N*, and the last hop's forward to the target becomes the `sock` that `ssh2-sftp-client.connect()` passes to ssh2 untouched (verified pass-through, no raw-client fallback). Hop clients are raw `ssh2.Client`s owned by the chain layer; each dial registers its own keyboard-interactive listener and host verifier honouring the same `interactive` flag as the target connect (interactive: provider `verify` + the pre-prompt timer; background: store-only `checkStored`, failing closed with `HostNotTrustedError`, and a keyboard-interactive hop fails before dialing with `VerificationRequiredError`). The keychain-retry constraint applies per hop too: after a rejected silent keychain answer the re-dial drops the known-rejected password and asks the user.
...
- **`onDidEvict` consumers** (Q34): `RemoteBrowserConnection` records its session's hop pool keys at connect, read from `SftpService.routeKeys` (`ChainConnectResult.hopKeys`, the keys actually dialed, config-derived hops included, 18b) rather than recomputed from the credential list; on a matching eviction it fires `onDidLoseRoute`, and `RemoteBrowserProvider` drops the panel to its existing Disconnected/click-to-reconnect state. Short-lived sessions just fail with `HopConnectError`; `SshTerminal` (feature 20) is the second consumer; see *SSH terminal* below.
...
**SSH terminal** (`src/terminal/SshTerminal.ts`, `src/commands/openSshTerminal.ts`, `src/ssh/dialClient.ts`, feature 20):

- `SshTerminal` implements `vscode.Pseudoterminal`. `open(dims)` writes the banner (`Connecting to <server> via <route>…`) at once and dials in the background: only then is the credential loaded with its secret, its route resolved (`resolveRoute`, `~/.ssh/config` applied and its `ProxyJump` chain, explicit jump hosts winning, notices reported, 18b), and `dialClient` asked for a raw ssh2 `Client`; the terminal never opens an SFTP session, so `getRawClient`/`execCommand` are not involved. The shell runs as `client.exec("cd -- '<path>' 2>/dev/null; exec \"${SHELL:-/bin/sh}\" -l", { pty: { term: 'xterm-256color', rows, cols } })` (R7: an exec session, not `shell()` plus a written `cd`; no banner race, no echoed command; the trade-off, documented in the guide, is that sshd skips MOTD and `~/.ssh/rc` for exec sessions and the command is POSIX-only). The path is single-quoted (`'` → `'\''`). `initialDimensions` may be `undefined` → 80×24 (R8-13); `setDimensions` → `setWindow`. Output goes through a per-stream `StringDecoder` so a multi-byte character split across chunks survives. `'exit'` records the status and `'close'` fires `onDidClose(code ?? 1)` at once (Q29); a dismissed prompt or any dial failure writes the message plus *Press any key to close this terminal* and delivers exit code 1 on the next keypress (R8-12, deferred: VS Code disposes an extension terminal the instant `onDidClose` fires, so an immediate close hid the message, manual §K finding); closing the tab aborts an in-flight dial through the terminal's own `AbortSignal` (input boxes are dismissed, host-key modals are not; accepted, the late verdict lands on a rejected dial).
- `dialClient(credential, { interactive, signal, openSock? })` is the vscode-free raw dial. It reuses the hop dialer (`createCredentialDialer`, exported from `chainConnect.ts`, the pool's `JumpHostDialer` generalised to take an `openSock` thunk instead of a pool handle), so a direct or chained target gets the registry's keyboard-interactive listener and host verifier, `DEFAULT_ALGORITHMS`, `tryKeyboard` + `readyTimeout: 0`, the pre-prompt timer, and the F8 rule: an auth failure after a silent keychain answer is retried once with the auto-answer disabled and the known-rejected password dropped. `openSock` is called again for the retry so a dead forward is never reused.
- Chains: when the credential has `jumpHosts`, `openSock` runs `chainConnect` (same pool, same per-hop prompts; the replacement chain is acquired before the previous one is released) and the terminal HOLDS the leases for the life of the shell; `drain()` marks the hop close-on-last-release and the shell's exit, a failure, or the tab closing releases it (Q25). `chainConnect` now also returns `hopKeys`, the leased hops' canonical pool keys; the terminal subscribes to `onDidEvict` and on a matching key tears the session down, writes `connection to <hop> lost`, and holds the tab for the keypress like any other failure (Q34). The same §K pass fixed a pre-existing panel bug: `RemoteBrowserProvider` now drops a navigated path (Go to Remote Path) whose listing fails or that belongs to a previous server (`navigatedServerId`), instead of retrying it on every render of every server until Reload Window. Agent forwarding is never requested (Q24; backlog: opt-in per terminal).
- Entry points (Q11), one core in `openSshTerminal(selection, deps)`: `fileferry.openSshTerminal` (palette → active server, `rootPath`), `fileferry.servers.openSshTerminal` (Servers tree `2_manage` → that server, `rootPath`), `fileferry.remoteBrowser.openSshTerminalHere` (Remote Files `remoteDirectory` context → the clicked folder) and `fileferry.remoteBrowser.openSshTerminalCurrentPath` (the panel `view/title` icon → the panel's current path). These are two commands, not one: VS Code passes the selected tree row to a title command that shares its id, which would override the current path with the selected folder (manual §K K7), the same split the createFile/createFileInCurrentPath pair uses. The tab is named `FileFerry: <server>, <path>`; FTP/FTPS servers are refused before anything is dialed; both tree-argument commands are hidden from the palette (L6). The banner's route comes from the non-secret credential list through the shared `describeRoute` (explicit hops, or the alias's `ProxyJump` chain, 18b; a deleted hop shows as `(missing jump host)`, then `chainConnect` fails on it). The pseudoterminal is driven in unit tests with fake ssh2 clients (the jest `vscode` mock provides `window.createTerminal`) and, at the wire level, against ssh2's in-process `Server`, direct and through a real bastion forward.
