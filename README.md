...
- **Upload on save**: auto-deploy on file save, toggled from the status bar; pin it on or off per server, so it stays armed for dev and off for production
...
## Jump hosts and a terminal, without an `ssh` binary

Reach servers that only a bastion can see, and open a shell there, using the same credentials and host-key verification as every deploy.

- **SSH jump hosts**: give a credential an ordered chain of hops in the Credentials panel and every connection to that server dials through them: deploys, Test Connection, diffs, backups, sync, and the Remote Files panel. Each hop authenticates on its own terms (password, key, agent, or 2FA) and is host-key verified
...
**`~/.ssh/config` aware**: tick "Resolve from `~/.ssh/config`" on a credential and reference a `Host` alias instead of a hostname; FileFerry reads HostName, Port, User, IdentityFile, and `ProxyJump` from your SSH config at connect time (SFTP).
...
Every SSH connection verifies the host key: the first connection to a server shows its fingerprint and prompts for trust (TOFU), and if a previously-trusted key changes, FileFerry refuses to connect until you accept the new one. Background triggers (upload on save, the watcher) never prompt; they fail fast with a warning instead.

`.vscode/fileferry.json` stores server references and path mappings, but no secrets. It is safe to commit.
